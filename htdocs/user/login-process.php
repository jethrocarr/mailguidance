...
if (user_online())
{
	...
	$_SESSION["error"]["username_mailguidance"] = "error";
	$_SESSION["error"]["password_mailguidance"] = "error";
...
else
{
	// check & convert input
	$instance	= NULL;
	$username	= security_form_input("/^[A-Za-z0-9.]*$/", "username_mailguidance", 4, "Please enter a username.");
	$password	= security_form_input("/^\S*$/", "password_mailguidance", 4, "Please enter a password.");
	...
	else
	{
		...
		if (!$_SESSION["error"]["message"])
		{
			...
			$_SESSION["error"]["username_mailguidance-error"] = 1;
			$_SESSION["error"]["password_mailguidance-error"] = 1;
		}

		if ($result == -3)
		{
			$_SESSION["error"]["instance_mailguidance-error"] = 1;
		}
